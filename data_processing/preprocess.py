...
def insert_dataframe_to_mysql(df, table_name, connection_config, batch_size = 1_000_000):
    ...
    column_names = ', '.join(f"{col}" for col in columns)  # Backticks for reserved words
    placeholders = ', '.join(['%s'] * len(columns))
    sql = f"INSERT INTO `{table_name}` ({column_names}) VALUES ({placeholders})"
    data = [tuple([element if not pd.isna(element) else None for element in row]) for row in df.values]
    conn = mysql.connector.connect(**connection_config)
    cursor = conn.cursor()
    
    # Insert in batches
    print(f"About to insert to {table_name}...")
    for i in tqdm(range(0, len(data), batch_size), total=len(data)//batch_size + 1, leave=False):
