...
export default async function movieRoutes(fastify, options) {
  ...
  fastify.post('/movies', { preHandler: verifyToken }, async (request, reply) => {
    ...
    if (!tconst || !primary_title) {
      return reply.code(400).send({ error: 'tconst and primary_title are required' });
    }
    ...
    try {
      ...
      await fastify.mysql.execute(
        ...
        [tconst, primary_title, release_year, runtime, average_rating, numvotes]
      );
  ...
  fastify.delete('/movies/:tconst', { preHandler: verifyToken }, async (request, reply) => {
    if (!request.user.isAdmin) {
      return reply.code(403).send({ error: 'Admin access required' });
    }
    ...
    try {
      ...
      await fastify.mysql.execute('DELETE FROM favorites WHERE tconst = ?', [tconst]);
      await fastify.mysql.execute('DELETE FROM genres WHERE tconst = ?', [tconst]);
      await fastify.mysql.execute('DELETE FROM director WHERE tconst = ?', [tconst]);
      await fastify.mysql.execute('DELETE FROM writer WHERE tconst = ?', [tconst]);
      await fastify.mysql.execute('DELETE FROM principal WHERE tconst = ?', [tconst]);
  ...
  fastify.put('/movies/:tconst', { preHandler: verifyToken }, async (request, reply) => {
    const { tconst } = request.params;
    const { primary_title, average_rating, release_year, runtime, numvotes } = request.body;
    ...
    try {
      const [result] = await fastify.mysql.execute(
        `UPDATE title 
         ...
         WHERE tconst = ?`,
        [primary_title, average_rating, release_year, runtime, numvotes, tconst]
      );
